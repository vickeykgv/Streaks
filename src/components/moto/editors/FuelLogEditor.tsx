...
export function FuelLogEditor({ id, vehicleId, onClose, onSaved }: FuelLogEditorProps) {
  ...
  const { register, handleSubmit, watch, setValue, reset, formState: { errors, isSubmitting } } = useForm<FuelLogFormValues>({
  ...
  const onSubmit = async (data: FuelLogFormValues) => {
    try {
      ...
      // Bump odometer on the vehicle if this reading is higher
    ...
    <div className="overflow-y-auto px-5 pb-6 pt-2">
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          <Field label="Litres" error={errors.litres?.message}>
            <input type="number" step="0.01" {...register('litres')} className={inputCls} placeholder="0.00" />
          </Field>
          <Field label="Price / L" error={errors.pricePerL?.message}>
            <input type="number" step="0.01" {...register('pricePerL')} className={inputCls} placeholder="0.00" />
          </Field>
          <Field label="Total cost" error={errors.totalCost?.message}>
            <input type="number" step="0.01" {...register('totalCost')} className={inputCls} placeholder="0.00" />
          </Field>
        </div>
        ...
        {/* Full tank toggle */}
        <div className="flex items-center justify-between rounded-2xl px-4 py-3" style={{ background: 'var(--bg-surface-2)' }}>
          <div className="flex items-center gap-2">
            <Zap size={15} color="var(--color-brand-500)" strokeWidth={2.4} />
            <div>
              <div className="font-sans text-[13px] font-bold text-[var(--text-primary)]">Full tank</div>
              <div className="font-body text-[11px] text-[var(--text-tertiary)]">Required for kmpl calculation</div>
            </div>
          </div>
          <button
            type="button"
        ...
        <Field label="Station — optional">
          <input {...register('station')} className={inputCls} placeholder="e.g. HP, Indian Oil…" />
        </Field>
        ...
        >
          {isSubmitting ? 'Saving…' : isEdit ? 'Save changes' : 'Log fill'}
        </button>
      </form>
